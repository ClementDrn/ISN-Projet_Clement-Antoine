...
class Hero:         # C'est la classe des carrés
    def __init__(self, num, x, y, c, co):      # __init__ est exécuté à la création d'un objet (carré), "self" est obligatoire laisse tomber
        self.num = num
        self.x = x         # Valeurs provisoire (x, y, size)
        self.y = y
        self.size = 50
        self.color = c
    ...
    def display(self):
        self.x = self.x       # x, y, s et c sont les attributs indiqués à la création des carrés (ligne 36)
        self.y = self.y       # ensuite on défini les variables du carré grâce aux attributs
        self.size = 50
        ...
        #     if self.vy >= 0:            # Si au-dessus..
            ...
            if self.y + self.size <= other.y:     # Si le bas du carré est au-dessus de l'autre..
                ...
                try:            # Pour éviter les divisions par 0
                    ratio = (self.y - ((get_can().coords(self.shape)[1]-get_oY())/get_ratio())
                            ) / (other.y - ((get_can().coords(self.shape)[3]-get_oY())/get_ratio()))        # Thalès pour trouver le point de collision en x
                    heroCollisionX = (self.x - ((get_can().coords(self.shape)[0]-get_oX())/get_ratio())
                            ) / ratio + ((get_can().coords(self.shape)[0]-get_oX())/get_ratio())
                except:
    ...
    def collide_block(self, block, other, isPlatform=False):
        if block.state == "solid":      # SOLID
            if self.x + self.size > block.x and self.x < block.x + block.sizeX:
                if self.y + self.size > block.y and self.y < block.y + block.sizeY:
                    if (get_can().coords(self.shape)[3]-get_oY())/get_ratio() <= block.y:              # hero provient d'en haut ?
                        ...
                        self.vy = 0
                    elif (get_can().coords(self.shape)[1]-get_oY())/get_ratio() >= block.y + block.sizeY:      # hero provient d'en bas ?
                    ...
                    elif (get_can().coords(self.shape)[2]-get_oX())/get_ratio() <= block.x:              # hero provient de la gauche ?
                        self.x = block.x - self.size
                        self.ax, self.vx = 0, 0
                    elif (get_can().coords(self.shape)[0]-get_oX())/get_ratio() >= block.x + block.sizeX:      # hero provient de la droite ?
                ...
                self.onBlock[block.num], self.onFloor = False, False
# SEMI-SOLID
        elif block.y <= self.y + self.size and self.vy >= 0:    # Si le hero passe..
            if (get_can().coords(self.shape)[3]-get_oY())/get_ratio() <= block.y:      #..à travers le bloc
                try:
                    ratio = (self.y - ((get_can().coords(self.shape)[1]-get_oY())/get_ratio())
                            ) / (block.y - ((get_can().coords(self.shape)[3]-get_oY())/get_ratio()))        # Thalès pour trouver le point de collision en x
                    collisionX = (self.x - ((get_can().coords(self.shape)[0]-get_oX())/get_ratio())
                            ) / ratio + ((get_can().coords(self.shape)[0]-get_oX())/get_ratio())
    ...
    def collide_platform(self, platform, other):
        self.collide_block(platform.block, other, True)
