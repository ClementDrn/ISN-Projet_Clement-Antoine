...
class Block(Stage):
    def __init__(self, state, x, y, sX, sY):
        super().__init__()
        self.x, self.y = x, y
...
class Platform(Block):
    def __init__(self, state, x, y, sX, sY, dx, dy):
        self.block = Block(state, x, y, sX, sY)
...
class Spawn(Stage):
    def _init_(self,x,y,hero):
        ...
        x2= self.x+ self.size
        y2= self.y+self.size
        get_can().coord( hero.shape,x1,y1,x2,y2 )
...
def create(level, hero1, hero2):              # Créé le niveau demandé
    global stage0
    ...
    for line in file:           # Prend chaque ligne une par une dans le fichier
        ...
        if line[0] == "block":
            if line[1] == "solid":
                otherObjs["solid"]["block" + str(count["blo"])] = Block(line[1], int(line[2]), int(line[3]), int(line[4]), int(line[5]))
                otherObjs["block"]["block" + str(count["blo"])] = otherObjs["solid"]["block" + str(count["blo"])]
            elif line[1] == "semi-solid":
                otherObjs["semi-solid"]["block" + str(count["blo"])] = Block(line[1], int(line[2]), int(line[3]),int(line[4]), int(line[5]))
    ...
    spawn1= Spawn(300,300,hero1)
    spawn2= Spawn(300,300,hero2)
