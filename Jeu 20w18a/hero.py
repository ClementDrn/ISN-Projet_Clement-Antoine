...
class Hero:         # C'est la classe des carrés
            ...
            if self.vy >= 0:            # Si au-dessus..
                if self.y + self.size < otherY:     # Si le bas du carré est au-dessus de l'autre..
                    ...
                    try:            # Pour éviter les divisions par 0
                        ratio = (self.y - get_can().coords(self.shape)[1]) / (otherY - get_can().coords(self.shape)[3])        # Thalès pour trouver le point de collision en x
    ...
    def collide_block(self, block):
        # print(get_can().coords(self.shape)[2])
        if self.x + self.size > block.x and self.x < block.x + block.sizeX:
            if self.y + self.size > block.y and self.y < block.y + block.sizeY:
                ...
                if get_can().coords(self.shape)[3] <= block.y:              # hero provient d'en haut ?
                    self.y = block.y - self.size
                    self.onBlock, self.onFloor = True, True         # L'avantage d'utiliser onBlock en plus : il ne peut être changé que ici contrairement à onFloor, donc on peut tester si avant le hero était sur le block
                    self.vy = 0
                elif get_can().coords(self.shape)[1] >= block.y + block.sizeY:      # hero provient d'en bas ?
                    self.y = block.y + block.sizeY + (block.y + block.sizeY - self.y) / 2
                    self.ay *= -0.5
        elif self.onBlock:
            self.onBlock, self.onFloor = False, False
